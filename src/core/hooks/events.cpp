...
void Hooks::Events::EventListener::FireGameEvent(IGameEvent *event) {
    if (strstr(event->GetName(), "player_hurt")) {
        ...
        if (attacker && victim && attacker == Globals::localPlayer) {
            ...
            if (CONFIGBOOL("Misc>Misc>Hitmarkers>Hitlogs")) {
                if (CONFIGBOOL("Legit>Backtrack>Backtrack") && Features::Backtrack::lastBacktrack > 4) {
                    Features::Notifications::addNotification(ImColor(255, 255, 255), "Backtracked %s %i ticks for %i health", info.name, Features::Backtrack::lastBacktrack, event->GetInt("dmg_health"));
                }
                else {
                    Features::Notifications::addNotification(ImColor(255, 255, 255), "Hit %s for %i health", info.name, event->GetInt("dmg_health"));
                }
            }
        ...
        if (attacker && victim && attacker == Globals::localPlayer) {
            ...
            if (CONFIGBOOL("Misc>Misc>Hitmarkers>Hitlogs")) {
                Features::Notifications::addNotification(ImColor(255, 255, 255), "Killed %s", info.name);
            }
        }
    }
